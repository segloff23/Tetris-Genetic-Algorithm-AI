...
# REQUIRED VARIABLES
model_names = ['weight', 'exponent']
model_sizes = [(None, 1), (None, 1)]
model_limits = [None, (0, 5)]
model_initializers = [norm_big, norm_abs_small]
model_dtypes = ['float32', 'float32']
model_mutators = [norm_small, norm_abs_small]

evaluator = lambda w, e, d, R : np.argmax(np.sum(w * np.power(np.abs(R), e),
                                                 axis=0))
